...
While the flowering season for our study cacti is in May, the cactus individual itself only blooms for 1-2 days throughout the season. Additionally, we are most interested in hummingbirds as pollinators, which are nearly impossible to document on camera traps and difficult to identify by eye in the field (especially female/juveniles). Because of these constraints, we will primarily rely on focal observations aided by a 200-500mm digital camera. We will do 1 hour observations in mornings or evenings at each combination of variables 4 times (36 hours of observations).

We will also deploy 18 unidirectional audio recorders with parabolic shields at 1 representation of every combination of cactus variables, which means recorders will be deployed at 9 sites (2 recorders per site). One recorder will face the cactus and one will face an open area in the opposite direction. The recorders will be 5m from the cactus of interest, and the open site will have no cacti within 5m of the recorded direction. After 5 days of recording, we will move the recorders to randomly selected cacti, and repeat the process for 25 days. 
